...
export default function OtpVerificationScreen({ route, navigation }) {
  // Destructure parameters including the passed OTP (testOtp) from Login/Register screens
  const { mobile, type, userId, countryCode, testOtp: passedOtp } = route.params || {};
  
  // Initialize OTP state with the passed OTP if it exists
  const [otp, setOtp] = useState(() => {
    const defaultOtp = ['', '', '', ''];
    if (passedOtp) {
  ...
  // --- TESTING STAGE STATES (Only for Resend Popup) ---
